...
openai.api_key = "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"
...
def generate_response(prompt):
    if "agregar medicamento" in prompt.lower():
        ...
        response = openai.Completion.create(
            ...
            prompt=f"Tu nombre es FocusFriend, eres un asistente médico virtual creado por Marco Acosta y Luis Galindo, eres servicial, tu principal función es consientizar a los pacientes acerca de la adherencia médica {prompt}",
            max_tokens=4000,
            n=1,
            stop=None,
            temperature=0.5,
...
def grabar_audio(seconds, fs, device_index_input):
    ...
    while True:
        ...
        with sr.AudioFile(filename) as source:
            ...
            try:
                ...
                if transcription is not None and (transcription.lower() == "focus" or transcription.lower() == "asistente"):
                    ...
                    speak_text("Hola, ¿en qué puedo servirte?")
                    pixels.think()
                    grabar_audio(seconds, fs, device_index_input)
        ...
        # Transcribe audio to text
                    ...
                    if response == "Dime el nombre del medicamento":
                        ...
                        medication_name=reconocer_audio(filename)
                        ...
                        #reconocer_audio(filename)
                        update_cell('FocusFriend_Registros',
                                        2, 3, reconocer_audio(filename))

                            # confirmation that the frequency was added
                            # convert_spanish_number(frequency)
                        response = f"Frecuencia de {medication_name} actualizada a cada {reconocer_audio(filename)} horas."
                        print(response)
                        speak_text(response)
                        send_message(
                                "+525520945995", f"Medicamento {medication_name} agregado correctamente, no olvides tomarlo cada {reconocer_audio(filename)} horas.")
                        time.sleep(25)
